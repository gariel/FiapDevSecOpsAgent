...
import { Calendar, GitCommit, User, ShieldAlert, ChevronRight, ArrowLeft } from "lucide-react";
...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    fetch(`/api/projects/${owner}/${repo}/${branch}`)
      .then(res => res.json())
      .then(data => {
      ...
      <header className="flex flex-col gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="bg-zinc-900 border border-zinc-800 px-2 py-0.5 rounded text-xs font-mono text-zinc-400">{branch}</span>
          </div>
        </div>
      </header>
            ...
            <div className="flex items-start gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-4 text-xs text-zinc-500">
                  ...
                  <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" />{new Date(scan.timestamp?._seconds * 1000).toLocaleString()}</span>
                </div>
              </div>
            </div>
